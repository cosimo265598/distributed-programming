...
void invio_file_to_client(int socket){
	...
	if(select(FD_SETSIZE,&cset,NULL,NULL,&tval)>0){
		...
		while (1)
		{
			// viene gestito il caso in cui ho richieste get separate e richieste get tutte di seguito
			...
			while (((ptr = strtok_r(rest,DELIM, &rest))!=NULL ) && ok ) {
				...
				if((fp=fopen(nomefile,"rb")) !=NULL)
				{
					...
					// invio del file a gruppi di 1 MByte alla volta 
					byte_file_letti=0;
					for(i=0;i<size;)
					{
						//printf("\nPrima della read");
...
int main (int argc, char *argv[])
{
	...
	while (1)
	{
		...
		// funzione che gestisce le richieste di strasmissione 
